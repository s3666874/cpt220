...
int main(void)
{
    ...
    /* by declaring the list I get memory for the list itself but not
     * for what is pointed to 
     */
    struct list list;
    /* initialise the list to a knows safe initial state */
    list_init(&list);

    /* ask for input of a new person until the user quits */
    while(!quit)
    {
        printf("Enter the person's first name: ");
        if(fgets(fname, NAMELEN+EXTRACHARS, stdin) == NULL || *fname=='\n')
        {
            quit=TRUE;
            break;
        }
        ...
        if(fgets(lname, NAMELEN+EXTRACHARS, stdin) == NULL || *lname == '\n')
        {
            quit=TRUE;
            break;
        }
